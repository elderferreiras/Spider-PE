...
public class JDialog_Spider_Login extends JDialog {
	...
	final static int GERENTE = 2;
	final static int RH = 3;
	
	static JDialog_Spider_Login home= new JDialog_Spider_Login();
	public static Spider_PE_Home Spider_PE_Home = null;
	public static Spider_PE_Settings Spider_PE_Settings = null;
	 ...
	 * Create the dialog.
	 */
	public JDialog_Spider_Login() {
		setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE);
		setModal(true);
		getContentPane().setBackground(Color.WHITE);
		//this.setUndecorated(true);
		setIconImage(Toolkit.getDefaultToolkit().getImage(JDialog_Spider_Login.class.getResource("/br/ufpa/spider/pe/img/config/favicon_pe.png")));
		try {
		...
		}
		{
			...
			jButtonSair.setIcon(new ImageIcon(JDialog_Spider_Login.class.getResource("/br/ufpa/spider/pe/img/config/off.png")));
		...
		}
		{
			...
			jButtonEntrar.setIcon(new ImageIcon(JDialog_Spider_Login.class.getResource("/br/ufpa/spider/pe/img/config/on.png")));
			jButtonEntrar.setActionCommand("Cancel");
		}
		populaComboBox();
		
		jTextFieldSenha.setText("123456");
		jTextFieldUsuario.setText("andrec");
...
>>>>>>> .r1058
	...
	private void administrador() {
		if(UsuarioDAO.findByLogin(jTextFieldUsuario.getText())==null){
			...
			if(login(usuario, jTextFieldSenha.getText())){
				...
				logado = ADMIN;
				Spider_PE_Settings = new Spider_PE_Settings();
	...
	private void gerenteProcesso() {
		if(GerenteProcessoDAO.findByLogin(jTextFieldUsuario.getText())==null){
			...
			if(login(humano, jTextFieldSenha.getText())){
				...
				JDialogWelcome.getInstance().setVisible(false);
				logado = GERENTE;
				Spider_PE_Home = new Spider_PE_Home();
				Spider_PE_Home.setVisible(true);
			}
			else JOptionPane.showMessageDialog(null, "Senha inv\u00E1lida.");		
		}
	}
	...
	public synchronized static JDialog_Spider_Login getInstance() {
		if (home == null){
			home = new JDialog_Spider_Login();
			}
		return home;
	}
