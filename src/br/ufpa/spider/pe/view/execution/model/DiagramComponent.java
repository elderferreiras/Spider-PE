    ...
    public String toString() {
    	if(Spider_PE_Home.getInstance().getLogado() == 3){
    		...
    		if(c.getType().equals(ComponentType.FORK) || c.getType().equals(ComponentType.JOIN)){
    			JoinFork join = JoinForkDAO.findById(c.getId(), Spider_PE_Home.getInstance().execution.getProcesso().getId());
    			if(join!=null)
    				return join.getTipo();
        	}
    	}
    		...
    		Component c = Modelling.getModelling().getComponent(getId());
    		if(c.getType().equals(ComponentType.FORK) || c.getType().equals(ComponentType.JOIN)){
    			JoinFork join = JoinForkDAO.findById(getId(), Spider_PE_Home.getInstance().Spider_PE_Management.getProcesso().getId());
    			if(join!=null)
    				return join.getTipo();
        	}
    	}      		
